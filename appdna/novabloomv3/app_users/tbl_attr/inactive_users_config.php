...
//A.I Notes  : max list col should be have 7 data important and relevant columns  , skip_cols_list the rest. eg if table have 20 cols  "skip_cols_list" => ["hive_site_id","hive_site_name", + the 13 cols ],  hive_site_id and hive_site_name should always be skipped in skip_cols_list skip_cols_profile
  ...
  $__page_title ="Inactive users";


  $modules_and_links_=[
    "profile_module_name"=>"viewdormant",
    "profile_module_link"=>"../../users/profile",
    "addnew_page_link"=>"../users/profile",
    "list_module_name"=>"dormarntusers",
    "list_page_link"=>"./inactive",
    "write_profile"=>true,
    "write_list"=>false
  ];
  ...
  $core_module_configs_=[
    ...
    "api_endpoint_name"=>"inactiveusers",
    "multigrid_col_span"=>"9"      

  ];
  
  $enforced_be_filter="accountStatus_not:'Active'";
...
//filterfile, title, table, column
  ...
  $list_btn_table_array=[
      $primary_table__=>[
         "calendar: Filter by Registration Date" => [
             "fe" => "filterByRegDate(`../users/dormarntusers`,`Filter registration date`, `app_users`,`createdAt`)",
             "be" => "filterByRegDate()",
  ...
  $profile_btn_table_array=[
      $primary_table__=>[
         ...
         "copy: add payment" => [
             "fe" => "addUserpayments({userRecordId:app_usersNode?.record_id, username:app_usersNode?.full_name})",
             "be" => "addUserpayments()",
             "file" => "user-payments"
         ]        
      ],
