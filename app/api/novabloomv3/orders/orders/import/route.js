...
import { AddOrders } from "../OrdersDbGateway";
...
export async function POST(req) {
  try {
    ...
    for (const node of processedInsertObj) {
      ...
      const mutatedObj = mutateInputArray("orders", node, req, newRecordId, authData);

     const insertResult = await AddOrders(newRecordId, mutatedObj, req, authData);
