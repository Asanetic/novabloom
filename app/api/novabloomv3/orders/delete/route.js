...
import { DeleteOrders } from '../orders/OrdersDbGateway';
...
export async function GET(request) {
  ...
  const deleteToken = searchParams.get('_orders_delete_record');
  const deleteTokenDecode = base64Decode(deleteToken);

  if (deleteToken) {
    // Customize table and WHERE clause here
    const table = 'orders'; // Replace with your actual table
    
     
    
    const whereStr = `WHERE primkey = '${deleteTokenDecode}'`;

    const res = await DeleteOrders(deleteTokenDecode, whereStr);
