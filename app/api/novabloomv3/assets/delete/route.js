...
import { DeleteDigitalassetlist } from '../digitalassetlist/DigitalassetlistDbGateway';
...
export async function GET(request) {
  ...
  const deleteToken = searchParams.get('_assets_delete_record');
  const deleteTokenDecode = base64Decode(deleteToken);

  if (deleteToken) {
    // Customize table and WHERE clause here
    const table = 'assets'; // Replace with your actual table
    
     
          
          const deleteAttachedMedia = await mosyQddata('assets', 'primkey', deleteTokenDecode);
    ...
    const res = await DeleteDigitalassetlist(deleteTokenDecode, whereStr);
