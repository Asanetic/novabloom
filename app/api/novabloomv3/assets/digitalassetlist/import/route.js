...
import { AddDigitalassetlist } from "../DigitalassetlistDbGateway";
...
export async function POST(req) {
  try {
    ...
    for (const node of processedInsertObj) {
      ...
      const mutatedObj = mutateInputArray("assets", node, req, newRecordId, authData);

     const insertResult = await AddDigitalassetlist(newRecordId, mutatedObj, req, authData);
