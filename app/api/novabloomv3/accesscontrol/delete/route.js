...
import { DeleteSystemmodulemanifest } from '../systemmodulemanifest/SystemmodulemanifestDbGateway';
...
export async function GET(request) {
  ...
  const deleteToken = searchParams.get('_system_module_manifest__delete_record');
  const deleteTokenDecode = base64Decode(deleteToken);

  if (deleteToken) {
    // Customize table and WHERE clause here
    const table = 'system_module_manifest_'; // Replace with your actual table
    
     
    
    const whereStr = `WHERE primkey = '${deleteTokenDecode}'`;

    const res = await DeleteSystemmodulemanifest(deleteTokenDecode, whereStr);
