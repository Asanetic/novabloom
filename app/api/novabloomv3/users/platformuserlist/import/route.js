...
import { AddPlatformuserlist } from "../PlatformuserlistDbGateway";
...
export async function POST(req) {
  try {
    ...
    for (const node of processedInsertObj) {
      ...
      const mutatedObj = mutateInputArray("app_users", node, req, newRecordId, authData);

     const insertResult = await AddPlatformuserlist(newRecordId, mutatedObj, req, authData);
