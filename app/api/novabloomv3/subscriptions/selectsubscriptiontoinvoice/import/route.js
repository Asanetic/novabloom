...
import { AddSelectsubscriptiontoinvoice } from "../SelectsubscriptiontoinvoiceDbGateway";
...
export async function POST(req) {
  try {
    ...
    for (const node of processedInsertObj) {
      ...
      const mutatedObj = mutateInputArray("subscriptions", node, req, newRecordId, authData);

     const insertResult = await AddSelectsubscriptiontoinvoice(newRecordId, mutatedObj, req, authData);
