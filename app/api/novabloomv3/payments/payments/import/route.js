...
import { AddPayments } from "../PaymentsDbGateway";
...
export async function POST(req) {
  try {
    ...
    for (const node of processedInsertObj) {
      ...
      const mutatedObj = mutateInputArray("payments", node, req, newRecordId, authData);

     const insertResult = await AddPayments(newRecordId, mutatedObj, req, authData);
