...
import { AddManageassetpricing } from "../ManageassetpricingDbGateway";
...
export async function POST(req) {
  try {
    ...
    for (const node of processedInsertObj) {
      ...
      const mutatedObj = mutateInputArray("asset_pricing", node, req, newRecordId, authData);

     const insertResult = await AddManageassetpricing(newRecordId, mutatedObj, req, authData);
