...
import { DeleteManageassetpricing } from '../manageassetpricing/ManageassetpricingDbGateway';
...
export async function GET(request) {
  ...
  const deleteToken = searchParams.get('_asset_pricing_delete_record');
  const deleteTokenDecode = base64Decode(deleteToken);

  if (deleteToken) {
    // Customize table and WHERE clause here
    const table = 'asset_pricing'; // Replace with your actual table
    
     
    
    const whereStr = `WHERE primkey = '${deleteTokenDecode}'`;

    const res = await DeleteManageassetpricing(deleteTokenDecode, whereStr);
