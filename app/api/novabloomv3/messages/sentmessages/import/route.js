...
import { AddSentmessages } from "../SentmessagesDbGateway";
...
export async function POST(req) {
  try {
    ...
    for (const node of processedInsertObj) {
      ...
      const mutatedObj = mutateInputArray("sent_messages", node, req, newRecordId, authData);

     const insertResult = await AddSentmessages(newRecordId, mutatedObj, req, authData);
