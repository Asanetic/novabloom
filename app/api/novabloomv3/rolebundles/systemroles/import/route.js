...
import { AddSystemroles } from "../SystemrolesDbGateway";
...
export async function POST(req) {
  try {
    ...
    for (const node of processedInsertObj) {
      ...
      const mutatedObj = mutateInputArray("system_role_bundles", node, req, newRecordId, authData);

     const insertResult = await AddSystemroles(newRecordId, mutatedObj, req, authData);
