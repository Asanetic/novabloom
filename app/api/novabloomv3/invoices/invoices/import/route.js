...
import { AddInvoices } from "../InvoicesDbGateway";
...
export async function POST(req) {
  try {
    ...
    for (const node of processedInsertObj) {
      ...
      const mutatedObj = mutateInputArray("invoices", node, req, newRecordId, authData);

     const insertResult = await AddInvoices(newRecordId, mutatedObj, req, authData);
