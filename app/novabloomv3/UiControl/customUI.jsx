...
export function MosyLiveSearch({
    ...
    const inputSignature = actionData?.inputSignature || `qdataInput`
    ...
    function handleOnSelect(dataRes) {
      ...
      if (actionName === "load_profile") {
        ...
        if (router && newQstr) {
            //router.push(`${actionData?.path}?${tableName}_mosyfilter=${btoa(newQstr)}`);
            window.location=`${actionData?.path}?${tableName}_dataNode=${btoa(newQstr)}`;
            stateSetters.setLocalEventSignature(magicRandomStr())
          closeMosyCard()
        }
    ...
    MosyCard(
      ...
      <>
        ...
        <LiveSearchDropdown
          ...
          hiddenInputName={inputSignature}
