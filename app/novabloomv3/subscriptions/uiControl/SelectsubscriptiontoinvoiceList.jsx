...
//data
import { loadSelectsubscriptiontoinvoiceListData, popDeleteDialog, InteprateSelectsubscriptiontoinvoiceEvent  } from '../dataControl/SelectsubscriptiontoinvoiceRequestHandler';
...
// Imports from invoice-details.jsx
import {
  subInvoiceHistory
} from '../logicControl/invoice-details';
...
export default function SelectsubscriptiontoinvoiceList({ dataIn = {}, dataOut = {} }) {
  ...
  useEffect(() => {
    ...
    loadSelectsubscriptiontoinvoiceListData(customQueryStr, stateItemSetters);
    
  }, [localEventSignature]);
  
  
  // Compute amount totals
  const sumsubscriptions_amount = stateItem.selectsubscriptiontoinvoiceListData?.reduce(
    (sum, row) => sum + Number(row.amount || 0),
    0
  );
  
  // Compute total_payments totals
  const sumsubscriptions_total_payments = stateItem.selectsubscriptiontoinvoiceListData?.reduce(
  ...
  return (
    ...
    <div className={`col-md-12  p-0 m-0  ${showDataControlSections && ("main_list_container")}  `} style={{marginTop: "0px", paddingBottom: "0px"}}>
      <form method="post" onSubmit={()=>{mosyFilterUrl({tableName:"subscriptions", keyword:stateItem.selectsubscriptiontoinvoiceQuerySearchStr})}} encType="multipart/form-data">
      
      {showDataControlSections && (<div className="row justify-content-end col-md-12 text-right pt-3 pb-3 data_list_section ml-0 mr-0 mb-3 border-bottom pr-0 pl-0" id="">
        <div className="col-md-6 p-0 text-left pt-3 hive_list_title">
          <h6 className="text-muted"><b> Select subscription to invoice </b></h6>
        </div>
        <div className="col-md-6 p-0 text-right hive_list_search_tray">
          <input type="text" id="txt_subscriptions" name="txt_subscriptions" className="custom-search-input form-control" placeholder="Search in Select subscription to invoice "
          onChange={(e) => stateItemSetters.setSelectsubscriptiontoinvoiceQuerySearchStr(e.target.value)}
          />
        ...
        <div className="row justify-content-end m-0 p-0 col-md-12 hive_list_action_btn_tray" id="">
          ...
          <div className="col-md-12 p-0 hive_list_nav_right_ribbon" id="">
            ...
            <MosyActionButton
            src="SelectsubscriptiontoinvoiceList"
            action="_filter_by_expiry_date"
            label=" Filter by expiry Date"
            icon="calendar"
            onClick={()=>{filterByBillingDate('../subscriptions/generateinvoice','Expiry date','subscriptions','next_billing_date')}}
            />
            ...
            icon="filter"
            onClick={()=>{filterByStats({router:router, stateSetters:stateItemSetters,path:'../subscriptions/generateinvoice'})}}
            />
            ...
            icon="users"
            onClick={()=>{filterByUser({router:router, stateSetters:stateItemSetters,path:'../subscriptions/generateinvoice'})}}
            />
            ...
            icon="bolt"
            onClick={()=>{filterByAsset({router:router, stateSetters:stateItemSetters,path:'../subscriptions/generateinvoice'})}}
            />
            
            <a href="generateinvoice" className="medium_btn border border_set btn-white hive_list_nav_refresh ml-3"><i className="fa fa-refresh mr-1 "></i> Refresh </a>
            
            
            <AddNewButton src="SelectsubscriptiontoinvoiceList" link={customProfilePath} label="New Subscription" icon="plus-circle" />
          </div>
        </div>
      </div> )}
      ...
      <div className="table-responsive  data-tables bottom_tbl_handler">
        ...
        <div className="text-left m-0 p-0 col-md-12">
          ...
          onClick={() => {mosyPrintToPdf({elemId : "subscriptions_print_card", defaultTitle:"Select subscription to invoice"})}}
          >
          <i className="fa fa-print "></i> Print List
        </div>
        <div className="cpointer p-2 ml-2 badge rounded border border_set badge-whte mb-3 tbl_print_to_excel_btn"
        
        onClick={() => exportTableToExcel("subscriptions_data_table", "Select subscription to invoice.xlsx")}
        >
    ...
    <div className="col-md-12 m-0 p-0" id="subscriptions_print_card">
      <table className="table table-hover  text-left printTarget" id="subscriptions_data_table">
        ...
        <tbody>
          {stateItem.selectsubscriptiontoinvoiceLoading ? (
            <tr>
              <th scope="col">#</th>
              <td colSpan="10" className="text-muted">
                <h5 className="col-md-12 text-center p-3 mb-5 text-muted"><i className="fa fa-spinner fa-spin"></i> Loading Select subscription to invoice ...</h5>
              </td>
            </tr>
          ) : stateItem.selectsubscriptiontoinvoiceListData?.length > 0 ? (
            stateItem.selectsubscriptiontoinvoiceListData.map((listsubscriptions_result, index) => {
                      ...
                      <div className="table_cell_dropdown">
                        <div className="table_cell_dropbtn">
                          ...
                          <div className="table_cell_dropdown-content">
                            ...
                            <MosyGridRowOptions
                            src="SelectsubscriptiontoinvoiceList"
                            action="_view_invoice_history"
                            label=" View invoice history"
                            icon="search"
                            dataIn={() => subInvoiceHistory(listsubscriptions_result.record_id)}   // only runs on click now
                            callBack={(incomingRequest) => {setChildDataOut(incomingRequest) }}
                            />
                          </div>
                        </div>
        ...
        src="SelectsubscriptiontoinvoiceList"
        tblName="subscriptions"
        totalPages={stateItem.selectsubscriptiontoinvoiceListPageCount}
