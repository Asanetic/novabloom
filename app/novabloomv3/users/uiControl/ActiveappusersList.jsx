...
//data
import { loadActiveappusersListData, popDeleteDialog, InteprateActiveappusersEvent  } from '../dataControl/ActiveappusersRequestHandler';
...
export default function ActiveappusersList({ dataIn = {}, dataOut = {} }) {
  ...
  useEffect(() => {
    ...
    loadActiveappusersListData(customQueryStr, stateItemSetters);
    
  }, [localEventSignature]);
  
  
  // Compute total_orders totals
  const sumapp_users_total_orders = stateItem.activeappusersListData?.reduce(
    (sum, row) => sum + Number(row.total_orders || 0),
    0
  );
  
  // Compute total_payments totals
  const sumapp_users_total_payments = stateItem.activeappusersListData?.reduce(
  ...
  return (
    ...
    <div className={`col-md-12  p-0 m-0  ${showDataControlSections && ("main_list_container")}  `} style={{marginTop: "0px", paddingBottom: "0px"}}>
      <form method="post" onSubmit={()=>{mosyFilterUrl({tableName:"app_users", keyword:stateItem.activeappusersQuerySearchStr})}} encType="multipart/form-data">
      
      {showDataControlSections && (<div className="row justify-content-end col-md-12 text-right pt-3 pb-3 data_list_section ml-0 mr-0 mb-3 border-bottom pr-0 pl-0" id="">
        <div className="col-md-6 p-0 text-left pt-3 hive_list_title">
          <h6 className="text-muted"><b> Active app users </b></h6>
        </div>
        <div className="col-md-6 p-0 text-right hive_list_search_tray">
          <input type="text" id="txt_app_users" name="txt_app_users" className="custom-search-input form-control" placeholder="Search in Active app users "
          onChange={(e) => stateItemSetters.setActiveappusersQuerySearchStr(e.target.value)}
          />
        ...
        <div className="row justify-content-end m-0 p-0 col-md-12 hive_list_action_btn_tray" id="">
          ...
          <div className="col-md-12 p-0 hive_list_nav_right_ribbon" id="">
            ...
            <MosyActionButton
            src="ActiveappusersList"
            action="_filter_by_registration_date"
            label=" Filter by Registration Date"
            icon="calendar"
            onClick={()=>{filterByRegDate(`../users/activelist`,`Filter registration date`, `app_users`,`createdAt`)}}
            />
            
            <a href="activelist" className="medium_btn border border_set btn-white hive_list_nav_refresh ml-3"><i className="fa fa-refresh mr-1 "></i> Refresh </a>
            
            
            <AddNewButton src="ActiveappusersList" link={customProfilePath} label="New User" icon="user-plus" />
          </div>
        </div>
      </div> )}
      ...
      <div className="table-responsive  data-tables bottom_tbl_handler">
        ...
        <div className="text-left m-0 p-0 col-md-12">
          ...
          onClick={() => {mosyPrintToPdf({elemId : "app_users_print_card", defaultTitle:"Active app users"})}}
          >
          <i className="fa fa-print "></i> Print List
        </div>
        <div className="cpointer p-2 ml-2 badge rounded border border_set badge-whte mb-3 tbl_print_to_excel_btn"
        
        onClick={() => exportTableToExcel("app_users_data_table", "Active app users.xlsx")}
        >
    ...
    <div className="col-md-12 m-0 p-0" id="app_users_print_card">
      <table className="table table-hover  text-left printTarget" id="app_users_data_table">
        ...
        <tbody>
          {stateItem.activeappusersLoading ? (
            <tr>
              <th scope="col">#</th>
              <td colSpan="10" className="text-muted">
                <h5 className="col-md-12 text-center p-3 mb-5 text-muted"><i className="fa fa-spinner fa-spin"></i> Loading Active app users ...</h5>
              </td>
            </tr>
          ) : stateItem.activeappusersListData?.length > 0 ? (
            stateItem.activeappusersListData.map((listapp_users_result, index) => {
        ...
        src="ActiveappusersList"
        tblName="app_users"
        totalPages={stateItem.activeappusersListPageCount}
