...
//data
import { loadAssetpricingListData, popDeleteDialog, InteprateAssetpricingEvent  } from '../dataControl/AssetpricingRequestHandler';
...
// ════════════════════════════════════════════════════════════════
// Imports from pricing-filters.jsx
import {
  filterByAsset
} from '../logicControl/pricing-filters';
...
export default function AssetpricingList({ dataIn = {}, dataOut = {} }) {
  
  //incoming data in from parent
  const {
    customQueryStr = "",
    customProfilePath="../asset_pricing/profile",
  ...
  useEffect(() => {
    ...
    loadAssetpricingListData(customQueryStr, stateItemSetters);
    
  }, [localEventSignature]);
  
  
  // Compute amount totals
  const sumasset_pricing_amount = stateItem.assetpricingListData?.reduce(
  ...
  return (
    ...
    <div className={`col-md-12  p-0 m-0  ${showDataControlSections && ("main_list_container")}  `} style={{marginTop: "0px", paddingBottom: "0px"}}>
      <form method="post" onSubmit={()=>{mosyFilterUrl({tableName:"asset_pricing", keyword:stateItem.assetpricingQuerySearchStr})}} encType="multipart/form-data">
      
      {showDataControlSections && (<div className="row justify-content-end col-md-12 text-right pt-3 pb-3 data_list_section ml-0 mr-0 mb-3 border-bottom pr-0 pl-0" id="">
        <div className="col-md-6 p-0 text-left pt-3 hive_list_title">
          <h6 className="text-muted"><b> Asset pricing </b></h6>
        </div>
        <div className="col-md-6 p-0 text-right hive_list_search_tray">
          <input type="text" id="txt_asset_pricing" name="txt_asset_pricing" className="custom-search-input form-control" placeholder="Search in Asset pricing "
          onChange={(e) => stateItemSetters.setAssetpricingQuerySearchStr(e.target.value)}
          />
        ...
        <div className="row justify-content-end m-0 p-0 col-md-12 hive_list_action_btn_tray" id="">
          ...
          <div className="col-md-12 p-0 hive_list_nav_right_ribbon" id="">
            ...
            <MosyActionButton
            src="AssetpricingList"
            action="_filter_by_asset"
            ...
            <AddNewButton src="AssetpricingList" link={customProfilePath} label="New asset pricing plan" icon="tag" />
          </div>
        </div>
      </div> )}
      ...
      <div className="table-responsive  data-tables bottom_tbl_handler">
        ...
        <div className="text-left m-0 p-0 col-md-12">
          ...
          onClick={() => {mosyPrintToPdf({elemId : "asset_pricing_print_card", defaultTitle:"Asset pricing"})}}
          >
          <i className="fa fa-print "></i> Print List
        </div>
        <div className="cpointer p-2 ml-2 badge rounded border border_set badge-whte mb-3 tbl_print_to_excel_btn"
        
        onClick={() => exportTableToExcel("asset_pricing_data_table", "Asset pricing.xlsx")}
        >
      ...
      <table className="table table-hover  text-left printTarget" id="asset_pricing_data_table">
        <thead className="text-uppercase">
          <tr>
            ...
            <th scope="col"><b>Billing period</b></th>
        ...
        <tbody>
          {stateItem.assetpricingLoading ? (
            <tr>
              <th scope="col">#</th>
              <td colSpan="9" className="text-muted">
                <h5 className="col-md-12 text-center p-3 mb-5 text-muted"><i className="fa fa-spinner fa-spin"></i> Loading Asset pricing ...</h5>
              </td>
            </tr>
          ) : stateItem.assetpricingListData?.length > 0 ? (
            stateItem.assetpricingListData.map((listasset_pricing_result, index) => {
                ...
                <Fragment key={`_row_${listasset_pricing_result.primkey}`}>
                  <tr key={listasset_pricing_result.primkey}>
                    <td>
                      ...
                      <td scope="col"><span title={listasset_pricing_result.total_subscriptions}>{magicTrimText(listasset_pricing_result.total_subscriptions, 70)}</span></td>
        ...
        src="AssetpricingList"
        tblName="asset_pricing"
        totalPages={stateItem.assetpricingListPageCount}
